...
public class DataAccess {
    ...
    public static double[][] getGroupPortfolio(int[] group_ids){
        if(group_ids[0]==0){
        ...
        try {
            ResultSet rs = null;
            String query;
            double[][] result = new double[group_ids.length][5];
            for(int i=0;i<group_ids.length;i++){
                query = "SELECT sellingValue,cashFlow,InvestMoney,MoneyWonOverall,netMoneyWonOverall FROM Groups WHERE groupId="+group_ids[i]+";";
                rs = stmt.executeQuery(query);
                rs.first();
                for(int j=0;j<5;j++){
    ...
    public static int[] getGroupIds(int clientId){
        try {
            ResultSet rs = null;
            String query = "SELECT COUNT(groupId) FROM GroupMember WHERE clientId="+clientId+";";
            rs = stmt.executeQuery(query);
            rs.first();
            int resultat = rs.getInt(1);
            int[] values;
            if(resultat > 0){
                values = new int[resultat];
    ...
    public static String getNameOfGroup(int groupId){
        try {
            ResultSet rs = null;
            String query = "SELECT COUNT(groupId) FROM Groups WHERE groupId="+groupId+";";
            rs = stmt.executeQuery(query);
            rs.first();
            int result = rs.getInt(1);
            rs.close();
...
public static String[][][][] getTransactionsOfGroups(int clientId){
        try {
            ...
            groupy = stmt.executeQuery(group_member_query);
            result = new String[group_number][][][];
            int[] group_id = new int[group_number];
            groupy.first();
            for(int g=0;g<group_number;g++){
                group_id[g] = groupy.getInt(1);
                groupy.next();
            }
       for(int g=0;g<group_number;g++){
            stocks_query_test = "SELECT COUNT(transactionId) FROM GroupStocksTransaction NATURAL JOIN Stock WHERE groupId="+group_id[g]+";";
            warrants_query_test = "SELECT COUNT(transactionId) FROM GroupWarrantsTransaction NATURAL JOIN Warrant NATURAL JOIN Stock WHERE groupId="+group_id[g]+";";
            trackers_query_test = "SELECT COUNT(transactionId) FROM GroupTrackerTransaction NATURAL JOIN Tracker NATURAL JOIN Stock WHERE groupId="+group_id[g]+";";


            stocks_query = "SELECT SName,operation,operationPrice,underlyingPrice,quantity,transactionTime FROM GroupStocksTransaction NATURAL JOIN Stock WHERE groupId="+group_id[g]+" ORDER BY(transactionTime)";

            warrants_query = "SELECT WName,SName,operation,operationPrice,optionPrice,underlyingPrice,quantity,transactionTime FROM GroupWarrantsTransaction NATURAL JOIN Warrant NATURAL JOIN Stock WHERE groupId="+group_id[g]+" ORDER BY(transactionTime)";

            trackers_query = "SELECT TName,SName,operation,operationPrice,optionPrice,underlyingPrice,quantity,transactionTime FROM GroupTrackerTransaction NATURAL JOIN Tracker NATURAL JOIN Stock WHERE groupId="+group_id[g]+" ORDER BY(transactionTime)";
